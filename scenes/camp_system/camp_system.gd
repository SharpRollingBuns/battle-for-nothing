...
const PLACE_PROBABILITIES = [0.1, 0.2, 0.4, 0.2, 0.1] # Для 1-5 мест соответственно
const MOB_PROBABILITIES = {
...
@onready var current_player_level = GameSession.player_state.level
...
func generate_enemy_party() -> Array:
	...
	for i in range(places):
		var enemy = _generate_single_enemy(places, enemy_party)
...
func _calculate_number_of_places() -> int:
	...
	var place_weights = []
	for i in range(5):
		var weight = PLACE_PROBABILITIES[i] * (1.0 + 0.1 * current_player_level)
...
func _generate_single_enemy(total_places: int, current_party: Array) -> Dictionary:
	...
	if current_party.size() > 0:
		...
		if has_skeleton:
			return {"type": "skeleton", "level": current_player_level}
	
	# 2. Друид может быть только в партии из 5 мест
	var mob_probabilities = MOB_PROBABILITIES.duplicate(true)
	if total_places < 5:
	...
	for mob in mob_probabilities:
		# Базовая вероятность
		var weight = mob_probabilities[mob][min(int(current_player_level / 20.0), 4)]
	...
	for mob in mob_weights:
		...
		if rand_val <= cumulative:
			return {"type": mob, "level": current_player_level}
